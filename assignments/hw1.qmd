...
1. Analyze the Olympic data set found [here](https://github.com/bbolker/stats720/blob/main/data/olymp1.csv) (raw download from [here](https://raw.githubusercontent.com/bbolker/stats720/main/data/olymp1.csv)).  The variables are:
...
```{r eval = FALSE}
library(tidyverse)
mydat |> filter(Medal == "Gold")  ## gold medals only
mydat |> group_by(Team, Year) |> summarise(across(n, sum)) ## medal count
mydat |> 
