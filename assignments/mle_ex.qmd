...
The beta-binomial is most usefully parameterized as

$$
\textrm{Prob}(x|p,\theta,N) = C(N,x) \frac{\textrm{Beta}(x+\theta p,N-x+\theta (1-p))}{\textrm{Beta}(\theta p,\theta(1-p))}
$$
...
Data from @Crowder1978 on germination of a parasitic plant:
...
- here we can give `mle2` a `data=` argument and use the columns of the data inside our log-likelihood expression
...
```{r mle2}
mle2(m ~ dbetabinom(prob = plogis(logitprob), size = n,
                    theta = exp(logtheta)),
