...
= & \llik(.) - \frac{1}{2} \log(\HH) + (q/2) \log(2\pi)
\end{split}
$$
...
\begin{split}
 ...
 = & \prod \int \llik(y_i|\bb_i,\beta) \llik(\bb_i|\theta) \, d\bb
\end{split}
$$
...
* still need to know curvature: choose quadrature knots based on $\hat bb$ and $1/H
...
obj$fn()
obj2 <- MakeADFun(ff, pars0, random = "b", silent = TRUE)
with(obj2, nlminb(par, fn, gr))
## check starting vals!
```
